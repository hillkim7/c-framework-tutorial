...
void communicate(asio::ip::tcp::socket& sock) {
	...
	// binary data.
	const char request_buf[] = "Hello ASIO\n";

	std::cout << "Sending: " << request_buf << std::endl;
	...
	if (ec == asio::error::eof) {
		...
		std::string s((std::istreambuf_iterator<char>(&response_buf)), std::istreambuf_iterator<char>());
...
int main(int ac, char* av[])
{
	if (ac != 2)
	{
		std::cerr << av[0] << " <server_ip>" << std::endl;
		return 1;
	}
